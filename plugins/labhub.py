...
class LabHub(DefaultConfigMixin, BotPlugin):
    ...
    def team_mapping(self):
        self.log.info(f'Org teams {self.TEAMS}')
        return {
    ...
    def is_room_member(invitee, msg):
        logger.info([member.username for member in msg.frm.room.occupants])
        return invitee in [member.username for member in msg.frm.room.occupants]
